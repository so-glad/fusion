...
var configSession = function configSession(config) {
    var redisBasic = config.databases.redis;

    if (config.session.store.indexOf('redis') === 0) {
        var sessionRedis = _lodash2.default.clone(redisBasic);
        if (config.session.store.length >= 7) {
            sessionRedis.db = config.session.store.substring(6);
        } else {
            sessionRedis.db = 0;
        }
        config.session.store = new _Redis2.default(sessionRedis);
    }
};
