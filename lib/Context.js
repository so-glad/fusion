...
var changePathVars = function changePathVars(config) {
    for (var key in config) {
        var value = config[key];
        if (_lodash2.default.isString(value)) {
            config[key] = value.replace('${path.client}', config.path.client).replace('${path.server}', config.path.server).replace('${path.resources}', config.path.resources);
